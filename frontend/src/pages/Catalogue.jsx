...
export default function Catalogue() {
  ...
  const nomDest = (id) =>
    destinations.find((d) => String(d.id) === String(id))?.nom || `#${id}`;
          ...
          <div style={s.filtersBar}>
            <div style={s.filterRow}>
              <select
                style={s.select}
                value={destIdHeb}
                onChange={(e) => setDestIdHeb(e.target.value)}
              >
              ...
              <div style={s.chips}>
                ...
                {TYPES_HEB.map((t) => (
                  <button
                    key={t}
                    style={typeHeb === t ? s.chipOn : s.chip}
                    onClick={() => setTypeHeb(t)}
                  >
          ...
          {loading ? (
            ...
            <div style={s.listWrap}>
              <div style={s.resultsCount}>
                {hebFiltres.length} hébergement
                {hebFiltres.length > 1 ? "s" : ""} trouvé
                {hebFiltres.length > 1 ? "s" : ""}
              </div>
              {hebFiltres.length === 0 ? (
                ...
                hebFiltres.map((h) => (
                  <div key={h.id} style={s.listCard}>
                    ...
                    <div style={s.listBody}>
                      ...
                      <div style={s.listSub}>
                        📍 {nomDest(h.destination_id)}
                      </div>
                      <div style={s.listDesc}>{h.description}</div>
                      <div style={s.tags}>
                        <span style={s.tag}>
                          {HEBERG_ICO[h.type]} {h.type}
                        </span>
      ...
      {tab === "transports" && (
        <>
          <div style={s.filtersBar}>
            <div style={s.chips}>
              <button
                style={typeTransport === "" ? s.chipOn : s.chip}
                onClick={() => setTypeTransport("")}
              >
                Tous les moyens
              </button>
              {TYPES_TRANS.map((t) => (
                <button
                  key={t}
                  style={typeTransport === t ? s.chipOn : s.chip}
                  onClick={() => setTypeTransport(t)}
                >
          ...
          {loading ? (
            ...
            <div style={s.listWrap}>
              <div style={s.resultsCount}>
                {transports.length} trajet{transports.length > 1 ? "s" : ""}{" "}
                disponible{transports.length > 1 ? "s" : ""}
              </div>
              {transports.length === 0 ? (
          ...
          <div style={s.filtersBar}>
            <div style={s.filterRow}>
              <select
                style={s.select}
                value={destIdAct}
                onChange={(e) => setDestIdAct(e.target.value)}
              >
                <option value="">🌍 Toutes les destinations</option>
                {destinations.map((d) => (
                  <option key={d.id} value={d.id}>
                ...
                activites.map((a) => (
                  <div key={a.id} style={s.listCard}>
                    ...
                    <div style={s.listBody}>
                      ...
                      <div style={s.listSub}>
                        📍 {nomDest(a.destination_id)}
                      </div>
                      <div style={s.listDesc}>{a.description}</div>
                      <div style={s.tags}>
                        <span style={s.tag}>⏱ {a.duree_heures}h</span>
                        <span
                          style={a.places_restantes === 0 ? s.tagFull : s.tag}
                        >
                          {a.places_restantes === 0
                            ? "Complet"
                            : `💺 ${a.places_restantes}/${a.capacite_max} places`}
                        </span>
                      </div>
                    </div>
                    <div style={s.listPrice}>
